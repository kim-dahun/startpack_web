...
const permissions = computed(() => sessionStore.getPermissions('GROUPS'))
...
const handleSaveGroup = async () => {
  if (!groupForm.groupId.trim() || !groupForm.groupName.trim()) {
    toast.add({
      severity: 'warn',
      summary: t('groups.requiredTitle'),
      detail: t('groups.requiredDetail'),
      life: 2200,
    })
    return
  }
...
const handleDeleteGroup = () => {
  ...
  confirm.require({
    message: t('groups.deleteConfirm', undefined, { name: selectedGroup.value.groupName }),
  ...
  <div class="page-stack">
    ...
    <template v-else>
      ...
      >
        <label class="inline-input">
          <span>{{ t('groups.field.serviceId') }}</span>
      ...
      <div class="admin-split-layout">
        ...
        <BaseEmptyState
          ...
          :title="t('menu.groups')"
          :description="t('groups.noSelectionDescription')"
        />
    ...
    <BaseDialog :visible="dialogVisible" :title="dialogMode === 'create' ? `${t('common.add')} ${t('menu.groups')}` : `${t('common.edit')} ${t('menu.groups')}`" @update:visible="dialogVisible = $event">
      <div class="form-popup-stack">
        <label class="inline-input">
          <span>{{ t('groups.field.groupId') }}</span>
          <InputText v-model="groupForm.groupId" />
        </label>
        <label class="inline-input">
          <span>{{ t('groups.field.groupName') }}</span>
          <InputText v-model="groupForm.groupName" />
        </label>
        <label class="inline-input">
          <span>{{ t('groups.field.description') }}</span>
