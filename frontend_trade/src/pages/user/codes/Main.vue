...
import { buildCodeColumns, createEmptyCodeGroupForm } from './model/pageModel'
...
const columns = ref(buildCodeColumns(t))
...
const syncCodeParentOptions = async () => {
  ...
  columns.value = buildCodeColumns(t).map((column) => {
    if (column.field === 'parentCodeId') {
      return {
        ...column,
        editorOptions: [{ label: t('common.notSelected'), value: '' }, ...options],
      }
    }
...
const handleSaveGroup = async () => {
  if (!groupForm.codeGroupId.trim() || !groupForm.codeGroupName.trim()) {
    toast.add({
      severity: 'warn',
      summary: t('codes.requiredTitle'),
      detail: t('codes.requiredDetail'),
      life: 2400,
    })
    return
  }
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: t('codes.savedTitle'),
      detail: t('codes.savedDetail'),
      life: 2200,
    })
  } finally {
...
const handleDeleteGroup = (codeGroupId: string) => {
  ...
  if (codeRows.value.length) {
    toast.add({
      severity: 'warn',
      summary: t('codes.deleteBlockedTitle'),
      detail: t('codes.deleteBlockedDetail'),
      life: 2400,
    })
    return
  }
  ...
  confirm.require({
    message: t('codes.deleteConfirmMessage', undefined, { name: target.codeGroupName }),
    header: t('codes.deleteConfirmTitle'),
    ...
    accept: async () => {
      ...
      toast.add({
        severity: 'success',
        summary: t('codes.deletedTitle'),
        detail: t('codes.deletedDetail'),
        life: 2200,
      })
    },
  })
...
<template>
  <div class="page-stack">
    <BasePageHeader title="menu.codes" />
    ...
    <template v-else>
      ...
      <div class="admin-split-layout">
        ...
        <BaseEmptyState
          ...
          :description="t('codes.emptyDescription')"
        />

        <BaseDataGrid
          v-else
          :caption="t('codes.gridCaption')"
          :columns="columns"
    ...
    <CodeGroupPopupView
      ...
      :title="dialogMode === 'create' ? t('codes.dialog.createTitle') : t('codes.dialog.editTitle')"
      :form="groupForm"
