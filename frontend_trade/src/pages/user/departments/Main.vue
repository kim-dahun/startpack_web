...
{ const trees = buildDepartmentTree(departments.value).map((node) => toOrgChartNode(node))
  console.log(trees)
  return trees;
},
)
...
const handleSaveDepartment = async () => {
  if (!departmentForm.departmentId.trim() || !departmentForm.departmentName.trim()) {
    toast.add({
      severity: 'warn',
      summary: t('departments.requiredTitle'),
  ...
  try {
    departments.value = await saveDepartments(
      dialogMode.value === 'edit'
        ...
        : { added: [{ ...departmentForm, comCd: comCd.value, departmentId: departmentForm.departmentId.trim(), departmentName: departmentForm.departmentName.trim() }], updated: [], deleted: [] },
      { comCd: comCd.value },
    )
    ...
    <template v-else>
      ...
      <div class="admin-split-layout">
        ...
        <BaseEmptyState
          ...
          :description="t('departments.noSelectionDescription')"
        />
