...
const getFavoritesStorageKey = (userId: string) => `frontend_trade.favorites.${userId}`
...
export const useSessionStore = defineStore('session', () => {
  const persisted = useStorage<SessionState>('frontend_trade.session', {
    user: null,
    token: null,
    menus: [],
    menuPermissions: [],
  ...
  const hasTradeAccess = computed(() => (persisted.value.serviceAccesses ?? []).includes(TRADE_SERVICE_ID))
  ...
  const login = async (payload: LoginRequest) => {
    ...
    if (!serviceAccesses.includes(TRADE_SERVICE_ID)) {
      throw new Error('TRADE 서비스 접근 권한이 없어 로그인할 수 없습니다.')
    }
    ...
    persisted.value = {
      ...
      serviceId: session.serviceId ?? TRADE_SERVICE_ID,
      serviceAccesses,
      groups,
    }

    if (session.token.accessToken) {
      window.localStorage.setItem('frontend_trade.accessToken', session.token.accessToken)
    } else {
      window.localStorage.removeItem('frontend_trade.accessToken')
    }
