...
const DEFAULT_SERVICE_ID = 'GROUPWARE'
...
export const useManagementScreen = ({
  ...
  const selectedServiceId = ref(sessionStore.persisted.serviceId ?? DEFAULT_SERVICE_ID)
  const keyword = ref('')
  ...
  const serviceOptions = computed(() => (
    adminScopedResourceKeys.has(resourceKey)
      ...
      : [sessionStore.persisted.serviceId ?? DEFAULT_SERVICE_ID]
  ))
  ...
  watch(
    ...
    async () => {
      const fallbackServiceId = sessionStore.persisted.serviceId ?? DEFAULT_SERVICE_ID
