...
const permissions = computed(() => sessionStore.getPermissions('DEPARTMENTS'))
...
const handleDeleteDepartment = () => {
  ...
  confirm.require({
    message: `${selectedDepartment.value.departmentName} 부서를 삭제합니다.`,
...
const handleDeleteMember = (row: UserPositionRow & { lockedHead?: boolean }) => {
  if (row.lockedHead) {
    toast.add({
      ...
      detail: '부서장으로 지정된 구성원은 삭제할 수 없습니다.',
      life: 2200,
    })
    return
  }
  confirm.require({
    message: `${row.userId} / ${row.positionId} 사용자를 부서에서 제외합니다.`,
    header: t('common.delete'),
    icon: 'pi pi-exclamation-triangle',
    acceptLabel: t('common.delete'),
    rejectLabel: t('common.close'),
  ...
  <div class="page-stack">
    ...
    <template v-else>
      ...
      <div class="admin-split-layout">
        ...
        <BaseEmptyState
          ...
          description="부서를 선택하면 소속 인원을 관리할 수 있습니다."
        />
    ...
    <BaseDialog :visible="dialogVisible" :title="dialogMode === 'create' ? `${t('common.add')} ${t('menu.departments')}` : `${t('common.edit')} ${t('menu.departments')}`" @update:visible="dialogVisible = $event">
      <div class="form-popup-stack">
        <label class="inline-input"><span>부서 ID</span><InputText v-model="departmentForm.departmentId" /></label>
        <label class="inline-input"><span>부서명</span><InputText v-model="departmentForm.departmentName" /></label>
        <label class="inline-input">
          <span>상위 부서</span>
        ...
        <label class="inline-input"><span>정렬 순서</span><InputNumber v-model="departmentForm.sortSeq" :min="1" fluid /></label>
