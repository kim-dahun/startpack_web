...
<template>
  <BaseDialog :visible="visible" :title="t('guide.departmentOrgChart')" @update:visible="emit('update:visible', $event)">
    <div class="org-chart-popup">
      <BaseEmptyState
        v-if="!orgChartNodes.length"
        title="No Departments"
        description="No department data is available for the current query."
      />
        ...
        <template #default="slotProps">
          <div class="department-org-card">
            ...
            <small v-if="slotProps.node.data?.departmentHeadUserId">
              Head: {{ String(slotProps.node.data.departmentHeadUserId) }}
            </small>
          </div>
        </template>
      </OrganizationChart>
