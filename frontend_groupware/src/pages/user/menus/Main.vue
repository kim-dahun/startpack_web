...
import { buildMenuTreeNodes, collectExpandedKeys, createEmptyMenuForm, filterMenuTree, flattenMenus, pageTitle } from './model/pageModel'
...
const dialogTitle = computed(() => {
  if (dialogMode.value === 'create-root') {
    return 'Add Root Menu'
  }
  if (dialogMode.value === 'create-child') {
    return 'Add Child Menu'
  }
  return 'Edit Menu'
})
...
const dialogParentLabel = computed(() => {
  ...
  return parentMenu ? `${parentMenu.menuName} (${parentMenu.menuId})` : 'ROOT'
})
...
const openCreateChild = (menuId: string) => {
  ...
  if (!parent || parent.menuLevel >= 3) {
    toast.add({
      severity: 'warn',
      summary: 'Cannot Add',
      detail: 'Menus can only have up to 3 levels.',
      life: 2400,
    })
    return
  }
...
const handleSave = async () => {
  if (!menuForm.menuId.trim() || !menuForm.menuName.trim()) {
    toast.add({
      ...
      detail: 'menuId and menuName are required.',
      life: 2400,
    })
    return
  }
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Saved',
      detail: 'Menu changes were saved.',
      life: 2200,
    })
  } finally {
...
const handleDelete = (menuId: string) => {
  ...
  if (target.children?.length) {
    toast.add({
      ...
      detail: 'Delete child menus first.',
      life: 2400,
    })
    return
  }
  ...
  confirm.require({
    message: `${target.menuName} will be deleted.`,
    header: 'Delete Menu',
  ...
  <div class="page-stack">
    ...
    <template v-else>
      ...
      <BaseEmptyState
        ...
        title="No Menus"
        description="There is no menu data for the current service."
      />
