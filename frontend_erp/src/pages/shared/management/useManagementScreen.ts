...
const manageableServiceIds = ['ERP', 'TRADE', 'GROUPWARE'] as const
...
export const useManagementScreen = ({
  ...
  const selectedServiceId = ref(sessionStore.persisted.serviceId ?? 'ERP')
  const keyword = ref('')
  ...
  const serviceOptions = computed(() => (
    adminScopedResourceKeys.has(resourceKey)
      ...
      : [sessionStore.persisted.serviceId ?? 'ERP']
  ))
  ...
  watch(
    ...
    async () => {
      const fallbackServiceId = sessionStore.persisted.serviceId ?? 'ERP'
