...
export const useUiStore = defineStore('ui', () => {
  const theme = useStorage<'light' | 'dark'>('frontend_erp.theme', 'light')
  const locale = useStorage<AppLocale>('frontend_erp.locale', 'ko')
  const sidebarOpen = useStorage('frontend_erp.sidebar-open', true)
  const sidebarMenuMode = useStorage<'all' | 'favorites'>('frontend_erp.sidebar-menu-mode', 'all')
  const expandedMenuIds = useStorage<string[]>('frontend_erp.sidebar-expanded-menu-ids', [])
