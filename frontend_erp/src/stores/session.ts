...
const ERP_SERVICE_ID = 'ERP'
const MANAGEABLE_SERVICE_IDS = ['TRADE', 'ERP', 'GROUPWARE'] as const
const getFavoritesStorageKey = (userId: string) => `frontend_erp.favorites.${userId}`
...
const normalizeMenus = (menus: SessionState['menus']) => menus.map((menu) => {
  const candidate = menu as typeof menu & { menuParentId?: string | null }
  return {
    ...menu,
    parentMenuId: menu.parentMenuId ?? candidate.menuParentId ?? null,
  }
})

export const useSessionStore = defineStore('session', () => {
  const persisted = useStorage<SessionState>('frontend_erp.session', {
    user: null,
    token: null,
    menus: [],
    menuPermissions: [],
  ...
  const hasErpAccess = computed(() => (persisted.value.serviceAccesses ?? []).includes(ERP_SERVICE_ID))
  ...
  const login = async (payload: LoginRequest) => {
    const session = await loginApi({
      ...payload,
      serviceId: ERP_SERVICE_ID,
    })
    const serviceAccesses = session.user.serviceAccesses ?? session.serviceAccesses ?? []
    const groups = session.groups ?? []

    if (!serviceAccesses.includes(ERP_SERVICE_ID)) {
      throw new Error('ERP 서비스 접근 권한이 없어 로그인할 수 없습니다.')
    }
    ...
    persisted.value = {
      ...
      token: session.token,
      menus: normalizeMenus(session.menus),
      menuPermissions: session.menuPermissions,
      serviceId: session.serviceId ?? ERP_SERVICE_ID,
      serviceAccesses,
      groups,
    }

    if (session.token.accessToken) {
      window.localStorage.setItem('frontend_erp.accessToken', session.token.accessToken)
    } else {
      window.localStorage.removeItem('frontend_erp.accessToken')
    }
